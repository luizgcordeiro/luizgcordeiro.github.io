...
%Equations use a different counter. the code that deals with ntags was modified. This is reset at sections
%END SPECIFIC FOR THIS FILE
%
...
function latextohtml(latex_file_input)
  ...
  while char_to_verify < length(file)
    ...
    if char_to_verify < length(file)-length(exp_to_verify) +2 && strcmp(file(char_to_verify:char_to_verify+length(exp_to_verify)-1),exp_to_verify)
	  ...
	  %SPECIFIC FOR THIS FILE
	  subsection_title=strrep(subsection_title,'--','&#8210;');
	  %END SPECIFIC FOR THIS FILE
    ...
    if char_to_verify < length(file)-length(exp_to_verify) +2 && strcmp(file(char_to_verify:char_to_verify+length(exp_to_verify)-1),exp_to_verify)
      ...
      if length(ntag_start)>0
        ...
        file = [file(1:ntag_start-1) ...
                  '\tag{' int2str(sec_num) '.' int2str(eq_counter) '}' ...
      ...
      if length(ntag_start)==0
        ...
        ref_name{alt_counter}=[int2str(sec_num) '.' int2str(eq_counter)];
      endif
    endif
    %
    ...
    if char_to_verify < length(file)-length(exp_to_verify) +2 && strcmp(file(char_to_verify:char_to_verify+length(exp_to_verify)-1),exp_to_verify)
      ...
      if strcmp(file(char_to_verify+length(exp_to_verify):char_to_verify+length(exp_to_verify)+length('[label=')-1),'[label=')
		...
		%SPECIFIC FOR THIS FILE
		%verify if there is a '\setcouter{enumi}{...}'
		if strcmp(file(label_close+1:label_close+length('\setcounter{enumi}{')),'\setcounter{enumi}{')
			list_css=[...
				'<style>ol.altlist' int2str(alt_counter) ' {counter-reset: listcounter ' file(label_close+length('\setcounter{enumi}{')+1:findclosingbrac(file,label_close+length('\setcounter{enumi}{'))-1) ...
				';}</style>'...
				list_css];
			
			file=[file(1:label_close) file(findclosingbrac(file,label_close+length('\setcounter{enumi}{'))+1:length(file))];
		endif
		%END SPECIFIC FOR THIS FILE
      endif
    ...
    if char_to_verify < length(file)-length(exp_to_verify) +2 && strcmp(file(char_to_verify:char_to_verify+length(exp_to_verify)-1),exp_to_verify)
		...
		file=[file(1:char_to_verify-1) ...
			sprintf('\n\n')...
			'<div class=' sprintf('''') 'quote' sprintf('''') '>' ...
    ...
    if char_to_verify < length(file)-length(exp_to_verify) +2 && strcmp(file(char_to_verify:char_to_verify+length(exp_to_verify)-1),exp_to_verify) 
      ...
      char_to_verify=char_to_verify+length(['<div class=' sprintf('''') envtype sprintf('''') str_id sprintf('>\n') ...
      '<span class=' sprintf('''') 'envidentifier' sprintf('''') '>'])-1;
  ...
  k=strfind(file,'\hyperref[');
  number_of_references=length(k);
  refbar=waitbar(0,'Creating hyperreferences...');
  announce=1;
  while length(k)>0
    open_sq_brac=k(length(k))+length('\hyperref[')-1;
    
    clos_sq_brac=findclosingbrac(file,open_sq_brac);
    
	open_brac=clos_sq_brac+1;
    clos_brac=findclosingbrac(file,open_brac);
	
    label=file(open_sq_brac+1:clos_sq_brac-1);
	ref_text=file(open_brac+1:clos_brac-1);
  ...
  while length(k)>0
    ...
    for i=1:length(citation_keys);
      ...
      str_citation=[str_citation oparg ']'];
    endfor
