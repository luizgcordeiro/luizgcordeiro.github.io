function convlist(originalfile,newfile)
...
while i<length(file)-length('\begin{')+1
	if file(i:i+7-1)=='\begin{' %this is to check if an environment has started at the i-th character
		...
		j=i+length('\begin{')-1;	%this is the position of the last character of '\begin{'
    
    j=findclosingbrac(file,j);
    %Now 'j' is the current closing bracket for the '\begin{...}'
    %anything in the middle will be the environment type
    envtype=file(i+7:j-1);
    
    %let us garantee we are in an 'enumerate' or 'itemize'
    if 1-strcmp(envtype,'enumerate') && 1-strcmp(envtype,'itemize')
      ...
      if file(j+1)=='['
        k=findclosingbrac(file,j+1);
        %Now, let us save the optional argument. If the optional argument is empty, this should break the program (which is not really a problem)
      ...
      if file(k+1:k+length('\label{'))=='\label{'
        p=findclosingbrac(file,k+7);  %position of the last character of '\label{'
...
towritenew=fopen(newfile,'w'); %create/empty file 'newfile'
file=strrep(file,'\','\\'); %technical details
file=strrep(file,'%','%%'); %technical details
fprintf(towritenew,file);
fclose(towritenew);
