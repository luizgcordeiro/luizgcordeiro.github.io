...
This approach -- to study an object by analyzing its <span>\&\#8220;</span>partial automorphisms<span>\&\#8221;</span> -- has been popularized in the contexts of Topological Dynamics and Operator Algebras mainly after the introduction of <em>partial actions</em> of groups by Exel on his study of the structure of $C^*$-algebras endowed with circle actions in \cite{MR1276163}. This turned out to be a rich an fruitful research direction, since it allows one to consider algebras and topological dynamical systems which are not induced (in any natural fashion) by a group action, but rather from transformations among its subsets (see \cite{arxiv1804.00396,MR3789176,MR2645883}, for example).

A variation on the <span>\&\#8220;</span>partial transformation<span>\&\#8221;</span> approach was taken by Sieben in \cite{MR1671944,MR1456588}, who instead considered <em>actions of inverse semigroups</em> on topological spaces and algebras. Inverse semigroups have been thoroughly studied in the last century, which provides solid foundation for this approach.

Another approach to Topological Dynamics and Operator Algebras, and which in fact predates the works above, is the usage of <em>groupoids</em>, which have played a central role in the theory of $C^*$-algebras since Renault's seminal work \cite{MR584266}, since they provide a geometric counterpart to a large class of (non-commutative) $C^*$-algebras. In simple terms, and as an interpretation of \cite[Theorem 5.9]{MR2460017}, groupoids are the <span>\&\#8220;</span>non-commutative spectra<span>\&\#8221;</span> of <span>\&\#8220;</span>non-commutative, dynamical $C^*$-algebras<span>\&\#8221;</span>.

Groupoids, partial actions of groups, and actions of inverse semigroups -- or more generally <em>partial actions of inverse semigroups</em> -- are related via the constructions of <span>\&\#8220;</span>groupoids of germs<span>\&\#8221;</span> and <span>\&\#8220;</span>crossed products<span>\&\#8221;</span>. We refer to \cite{MR2045419,arxiv1804.00396,MR3743184,MR3851326,MR2799098,MR1724106,MR1671944,MR2565546}. In fact, groupoids and inverse semigroups may be seen as dual to each other, via the <em>non-commutative Stone duality</em> of Lawson-Lenz, \cite{MR3077869}. We also refer to \cite{MR2969047,MR2644910,MR2304314} for related studies.

Actions of groupoids have also appeared throughout the literature, in the context of Lie theory (\cite{MR2012261}), algebraic topology (\cite{MR2273730}), operator algebras and topological dynamics (\cite{MR2982887,MR2969047,MR2941279}). A generalization of groupoids and inverse monoids, called <em>inverse categories</em>, were initially introduced in \cite{MR0506554} and have been considered in recent work, e.g.\ in the study of <em>tilings</em> (\cite{MR1736698}), logic and recursion theory (\cite{MR1871071,MR3605681}) and crossed product algebras (\cite{MR2959793}).

The main goal of this article is to study the structure of <em>inverse semigroupoids</em>, which are a common generalization of both inverse semigroups and groupoids. First we begin by comparing the notions of semigroupoid used in \cite{MR3597709,MR915990} to that of \cite{MR2754831}, which is more general. After that, we introduce inverse semigroupoids and prove some basic facts about their structure (some which had already been proven in \cite{MR3597709}). We follow with a representation theorem, which is an analogue of Cayley's Theorem for groups and the Vagner-Preston theorem for inverse semigroups. It motivates a notion of actions -- and more generally $\land$-preactions and partial actions -- of inverse semigroupoids on semigroupoids, in a manner which covers previously used notions for inverse semigroups and for groupoids. These are used to construct <span>\&\#8220;</span>semidirect products<span>\&\#8221;</span> which generalize transformation groupoids of partial group actions, semidirect products of inverse semigroups, semidirect products of groupoids, among others.
...
We assume familiarity with inverse semigroups and étale groupoids. The main references are \cite{MR1694900,MR1724106} (see also the author's PhD thesis \cite{cordeirothesis}). A <em>semilattice</em> is a poset $(P,\leq)$ admiting binary meets (infima), and is always regarded as an inverse semigroup under meets: $ab=a\land b=\inf\left\{a,b\right\}$ for all $a,b\in P$.

<h3 class='subsection'>1.1. Setoids, bundles and partitions</h2>
    <div class='alt'>
...
<h3 class='subsection'>1.2. Graphs</h2>
...
<span class='envidentifier'>Definition</span> <span class='counter'>1.1</span>
...
If $G$ and $H$ are graphs over the same vertex set $G^{(0)}=H^{(0)}$, we make the fibred product $G\tensor[_{\so}]{\ast}{_{\ra}} H$ into a graph over that same vertex set, with source and range maps $\ra(g,h)=\ra(g)$ and $\so(g,h)=\so(h)$.  The construction of graphs of paths obey the <span>\&\#8220;</span>rules of exponentiation<span>\&\#8221;</span>, where the fibred product $\tensor[_{\so}]{\ast}{_{\ra}}$ takes the role of the product: given $k,p\in\mathbb{N}_{\geq 0}$, we have natural isomorphisms
<div class='equation'>
\begin{equation*}
...
<h2 class='section'>2. Inverse semigroupoids</h2>

    <h3 class='subsection'>2.1. Exel and graphed semigroupoids</h2>
...
<!-- 2.2 -->
<div class='definition' id='def:semigroupoid'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.2</span> <span class='envop'>({\cite[Definition 2.1]{MR2754831}})</span>
...
<ol class='alt altlist1'>
    \label{def:semigroupoiditem1} $(f,g)\in\Lambda^{[2]}$ and $(g,h)\in\Lambda^{[2]}$;
    \label{def:semigroupoiditem2} $(f,g)\in\Lambda^{[2]}$ and $(fg,h)\in\Lambda^{[2]}$;
    \label{def:semigroupoiditem3} $(g,h)\in\Lambda^{[2]}$ and $(f,gh)\in\Lambda^{[2]}$;
</ol>
...
We use square brackets when denoting the subset $\Lambda^{[2]}$ of $\Lambda\times\Lambda$ to stress the fact that $\Lambda$ has no graph structure. Moreover, instead of saying that a pair $(f,g)$ belongs to $\Lambda^{[2]}$ we may simply state that <span>\&\#8220;</span>the product $fg$ is (well-)defined<span>\&\#8221;</span>.
...
<span class='envidentifier'>Example</span> <span class='counter'>2.3</span>
...
This is an alternative to the more useful construction of a <em>transformation groupoid</em>, which is a particular case of a semidirect product of semigroupoids (see Subection \ref{sec:dualprehomomorphisms}).
</div>
...
<span class='envidentifier'>Example</span> <span class='counter'>2.4</span>
...
<span class='envidentifier'>Example</span> <span class='counter'>2.5</span>
...
<span class='envidentifier'>Example</span> <span class='counter'>2.6</span>
...
<span class='envidentifier'>Example</span> <span class='counter'>2.7</span>
...
<span class='envidentifier'>Definition</span> <span class='counter'>2.8</span>
...
<span class='envidentifier'>Definition</span> <span class='counter'>2.9</span>
...
<!-- 2.10 -->
<div class='example' id='ex:imageofhomomorphismisnotasubsemigroupoid'>
<span class='envidentifier'>Example</span> <span class='counter'>2.10</span>
...
<!-- 2.11 -->
<div class='definition' id='def:graphedsemigroupoid'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.11</span> <span class='envop'>({\cite[p.\ 194]{MR915990}})</span>
...
<ol class='alt altlist2'>
    \label{def:graphedsemigroupoiditem1} for all $(a,b)\in \mathcal{S}^{(2)}$, $\so(ab)=\so(b)$ and $\ra(ab)=\ra(a)$ -- i.e., $(\id_{\mathcal{S}^{(0)}},\mu)$ is a graph morphism;
    \label{def:graphedsemigroupoiditem2} for all $(a,b,c)\in \mathcal{S}^{(3)}$, $(ab)c=a(bc)$.
</ol>
...
<!-- 2.12 -->
<div class='proposition' id='prop:graphedsemigroupoidsaresemigroupoids'>
<span class='envidentifier'>Proposition</span> <span class='counter'>2.12</span>
...
<ol class='alt altlist3'>
    \label{prop:graphedsemigroupoidsaresemigroupoidsitem1} $(\mathcal{S}^{(0)},\mathcal{S},\so,\ra,\mu)$ is a graphed semigroupoid;
    \label{prop:graphedsemigroupoidsaresemigroupoidsitem2} $(\mathcal{S},\mathcal{S}^{(2)},\mu)$ is an Exel semigroupoid.
    \label{prop:graphedsemigroupoidsaresemigroupoidsitem3} for all $(a,b)\in \mathcal{S}^{(2)}$, $\so(ab)=\so(b)$ and $\ra(ab)=\ra(b)$.
</ol>
...
<!-- 2.13 -->
<div class='proof' id='proof13'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.13</span>
...
<span class='envidentifier'>Example</span> <span class='counter'>2.14</span>
...
<!-- 2.15 -->
<div class='definition' id='definition15'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.15</span> <span class='envop'>({\cite[Definition 19.1]{MR2419901}})</span>
...
<!-- 2.16 -->
<div class='proposition' id='prop:equivalencecategorical'>
<span class='envidentifier'>Proposition</span> <span class='counter'>2.16</span>
The following are equivalent for an Exel semigroupoid $\Lambda$:
<style>ol.altlist4{
content:  counter(listcounter);
}
</style>
<ol class='alt altlist4'>
  \label{prop:equivalencecategoricalitem1} $\Lambda$ is categorical.
  \label{prop:equivalencecategoricalitem2} For all $a,b\in\Lambda$, $\Lambda_a$ and $\Lambda_b$ are either disjoint or equal.
</ol>
</div>
<!-- 2.17 -->
<div class='proof' id='proof17'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.17</span>
...
We will now prove that categorical semigroupoids are precisely those with a compatible graph structure. All that is needed is to define an appropriate vertex set and source and range maps. The main idea is that if $\mathcal{S}$ is a graphed semigroupoid and $(a,z)\in\mathcal{S
    ...
    }^{(2)}$), then we add a <span>\&\#8220;</span>dummy<span>\&\#8221;</span> vertex as the source of $a$.

<!-- 2.18 -->
<div class='theorem' id='prop:equivalencegraphableandcategoricalsemigroupoids'>
<span class='envidentifier'>Theorem</span> <span class='counter'>2.18</span>
...
<!-- 2.19 -->
<div class='proof' id='proof19'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.19</span>
...
Consider two collections of symbols <div class='equation'>
\begin{equation*}
...
<div class='equation' id='eq:additionalpropertyofequivalencerelations'>
\begin{equation*}
\text{if }\Lambda^b=\varnothing\text{ then }(v_0(b),v_0(ab))\in R_0\qquad\text{and}\qquad\text{if }\Lambda_a=\varnothing\text{ then }(v_1(a),v_1(ab))\in R_1.\tag{2.20}
\end{equation*}
</div>
...
<!-- 2.21 -->
<div class='proposition' id='prop:classificationofgraphsoncategorical'>
<span class='envidentifier'>Proposition</span> <span class='counter'>2.21</span>
...
<!-- 2.22 -->
<div class='proof' id='proof22'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.22</span>
Consider the sets <div class='equation'>
\begin{equation*}
...
<!-- 2.23 -->
<div class='itemize' id='itemize23'>
<span class='envidentifier'>Itemize</span> <span class='counter'>2.23</span>
...
<!-- 2.24 -->
<div class='itemize' id='itemize24'>
<span class='envidentifier'>Itemize</span> <span class='counter'>2.24</span>
...
<span class='envidentifier'>Example</span> <span class='counter'>2.25</span>
...
We finish this introduction by providing a condition that allows us to <span>\&\#8220;</span>extend<span>\&\#8221;</span> semigroupoid homomorphisms between graphed semigroupoids to graph homomorphisms.

<!-- 2.26 -->
<div class='proposition' id='prop:vertexmap'>
<span class='envidentifier'>Proposition</span> <span class='counter'>2.26</span>
Suppose that $G$ and $H$ are graphs, that $G$ has no sources nor sinks, and that $\phi\colon G\to H$ is a map such that $(\phi\times\phi)(G^{(2)})\subseteq H^{(2)}$. Then there exists a unique <span>\&\#8220;</span>vertex map<span>\&\#8221;</span> $\phi^{(0)}\colon G^{(0)}\to H^{(0)}$ such that $(\phi^{(0)},\phi)$ is a graph morphism.
</div>
<!-- 2.27 -->
<div class='proof' id='proof27'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.27</span>
...
<!-- 2.28 -->
<div class='corollary' id='cor:graphedhomomorphisminducesvertexmap'>
<span class='envidentifier'>Corollary</span> <span class='counter'>2.28</span>
Suppose that $\mathcal{S}$ and $\mathcal{T}$ are graphed semigroupoids, $\phi\colon\mathcal{S}\to\mathcal{T}$ is a semigroupoid homomorphism, and that $\mathcal{S}$ has no sources nor sinks. Then there exists a unique <span>\&\#8220;</span>vertex map<span>\&\#8221;</span> $\phi^{(0)}\colon\mathcal{S}^{(0)}\to\mathcal{T}$ such that $(\phi^{(0)},\phi)$ is a graph homomorphism.
</div>

<!-- 2.29 -->
<div class='example' id='example29'>
<span class='envidentifier'>Example</span> <span class='counter'>2.29</span>
The proposition above is not valid if we just assume that $\mathcal{S}$ has no sinks (or no sources). Let us associate a semigroupoid $A$ to the <em>strict</em> order of $\mathbb{N}$ in the same manner as we associate categories to (non-strict) orders -- namely $A=\left\{(n,m)\in\mathbb{N}\times\mathbb{N}:m<n\right\}$, with product $(n,m)(m,k)=(n,k)$, vertex set $A^{(0)}=\mathbb{N}$ and source and range maps $\so(n,m)=m$, $\ra(n,m)=n$.
...
Then neither $\mathcal{S}_1$ nor $\mathcal{S}_2$ have sinks, but the identity map $\mathcal{S}_2\to\mathcal{S}_1$ is an Exel semigroupoid isomorphism which cannot be extended to a graph homomorphism.
    ...
    <h3 class='subsection'>2.2. Inverse semigroupoids</h2>
...
<!-- 2.30 -->
<div class='definition' id='def:inversesemigroupoid'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.30</span>
...
<div class='equation' id='eq:inversesemigroupoid'>
\begin{equation*}
...
<span class='envidentifier'>Example</span> <span class='counter'>2.31</span>
...
<!-- 2.32 -->
<div class='lemma' id='lem:exelinverseiscategorical'>
<span class='envidentifier'>Lemma</span> <span class='counter'>2.32</span>
Let $\Lambda$ be an inverse semigroupoid. The following are equivalent:
<style>ol.altlist5{
content:  counter(listcounter);
}
</style>
<ol class='alt altlist5'>
\label{lem:exelinverseiscategorical1} $\Lambda$ is categorical;
\label{lem:exelinverseiscategorical2} For all $e,f\in E(\Lambda)$, if $ef$ is defined, then $fe$ is also defined and $ef=fe$.
</ol>
</div>
<!-- 2.33 -->
<div class='proof' id='proof33'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.33</span>
...
The set $\Lambda^{(0)}$ constructed above is the <em>space of germs</em> of $E(\Lambda)$ with its canonical order. This construction will be further developed in Subsection \ref{subsec:groupoidofgerms}.

<!-- 2.34 -->
<div class='theorem' id='theo:everyinversesemigroupoidiscategorical'>
<span class='envidentifier'>Theorem</span> <span class='counter'>2.34</span>
...
<!-- 2.35 -->
<div class='proof' id='proof35'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.35</span>
...
\end{equation*}
  ...
  <div class='equation' id='eq:exelinverseiscategorical'>
\begin{equation*}
(fxe)(ef)(fxe)=fx(e^2)(f^2)xe=fxefxe=fxe\tag{2.36}
\end{equation*}
</div>
...
<!-- 2.37 -->
<div class='corollary' id='cor:inversesemigroupoidhasauniquegraphstructure'>
<span class='envidentifier'>Corollary</span> <span class='counter'>2.37</span>
...
<!-- 2.38 -->
<div class='definition' id='definition38'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.38</span>
...
<!-- 2.39 -->
<div class='proposition' id='prop:propertiesoperationsofinversesemigroupoid'>
<span class='envidentifier'>Proposition</span> <span class='counter'>2.39</span>
Let $\mathcal{S}$ be an inverse semigroupoid. Then
<style>ol.altlist6{
content:  counter(listcounter,lower-alpha);
}
</style>
<ol class='alt altlist6'>
    \label{prop:propertiesoperationsofinversesemigroupoid1} $(ab)^*=b^*a^*$, for all $(a,b)\in\mathcal{S}^{(2)}$.
    \label{prop:propertiesoperationsofinversesemigroupoid2} If $e\in E(\mathcal{S})$ and $(b,e)\in \mathcal{S}^{(2)}$. then $beb^*\in E(\mathcal{S})$.
    \label{prop:propertiesoperationsofinversesemigroupoid3} the following are equivalent:
    <style>ol.altlist7{
content:  counter(listcounter,lower-roman);
}
</style>
<ol class='alt altlist7'>
        \label{prop:propertiesoperationsofinversesemigroupoid31} $a\leq b$;
        \label{prop:propertiesoperationsofinversesemigroupoid32} there exists $e\in E(\mathcal{S})$ such that $a=be$;
        \label{prop:propertiesoperationsofinversesemigroupoid33} there exists $f\in E(\mathcal{S})$ such that $a=fb$;
        \label{prop:propertiesoperationsofinversesemigroupoid34} $a^*\leq b^*$.
    </ol>
    \label{prop:propertiesoperationsofinversesemigroupoid4} $\leq$ is a partial order on $\mathcal{S}$;
    \label{prop:propertiesoperationsofinversesemigroupoid5} If $a\leq b$, $c\leq d$ and $(a,c)\in \mathcal{S}^{(2)}$, then $ac\leq bd$.
</ol>
</div>
...
<!-- 2.40 -->
<div class='itemize' id='itemize40'>
<span class='envidentifier'>Itemize</span> <span class='counter'>2.40</span>
    ...
    <h3 class='subsection'>2.3. A representation theorem</h2>
...
Representation results have been obtained in the context of <em>inverse categories</em>, i.e., (possibly large) categories which behave as inverse semigroupoids. Namely, it is proven that every (locally small) inverse category may be faithfully embedded into the category $\cat{PInj}$ of sets and partial bijections. For more details, see \cite[p.\ 87]{MR0506554}, \cite[Theorem 3.8]{MR1871071}, \cite[Proposition 3.11]{MR3093088}. However we adopt a fundamentally different perspective from those works, by considering inverse semigroupoids as innate to bundles. This approach has the advantage of making the dynamical nature of inverse semigroupoids more explicit, and in particular to naturally motivating a natural notion of <em>action</em> for inverse semigroupoids.
...
%<!-- 2.41 -->
<div class='proposition' id='proposition41'>
<span class='envidentifier'>Proposition</span> <span class='counter'>2.41</span>
%$\mathcal{I}(\pi)$ is an inverse semigroupoid.
%</div>
%<!-- 2.42 -->
<div class='proof' id='proof42'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.42</span>
...
<!-- 2.43 -->
<div class='example' id='example43'>
<span class='envidentifier'>Example</span> <span class='counter'>2.43</span>
...
<!-- 2.44 -->
<div class='example' id='example44'>
<span class='envidentifier'>Example</span> <span class='counter'>2.44</span>
...
<!-- 2.45 -->
<div class='theorem' id='thm:representationtheoremoid'>
<span class='envidentifier'>Theorem</span> <span class='counter'>2.45</span>
...
<!-- 2.46 -->
<div class='proof' id='proof46'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.46</span>
...
<div class='equation' id='eq:equivalenceproductdefinedivagnerprestonoid'>
\begin{equation*}
ab\text{ is defined}\iff\so(a)=\ra(b)\iff\so(\alpha(a))=\ra(\alpha(b))\iff\alpha(a)\alpha(b)\text{ is defined}.\tag{2.47}
\end{equation*}
</div>
...
Still considering the map $\alpha$ of the proof above, note that the vertex set of $\mathcal{I}(\ra)$ is $\mathcal{S}^{(0)}$, the vertex set of $\mathcal{S}$. We have $\so\circ\alpha=\so$, so the <span>\&\#8220;</span>vertex map<span>\&\#8221;</span> (as in Corollary \ref{cor:graphedhomomorphisminducesvertexmap}) associated to $\alpha$ is the identity of $\mathcal{S}^{(0)}$. This motivates the definition of an <em>action</em> of an inverse semigroupoid $\mathcal{S}$ on a set $X$.

<!-- 2.48 -->
<div class='definition' id='def:action'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.48</span>
...
However, as stated in the introduction, we will be interested in more general notions than actions, which is the content of the next subsection.
    ...
    <h3 class='subsection' id='sec:dualprehomomorphisms'>2.4. \texorpdfstring{$\land$}{∧}-prehomomorphisms, partial homomorphisms, and actions</h2>
    
    Generalizations of homomorphisms, initially called $\lor$ and $\land$-<em>prehomomorphisms</em>, were respectively introduced in \cite[Definition 1.1]{MR0424979} and \cite[Definition 4.1]{MR0470123} by McAlister and Reilly on their study of $E$-unitary inverse semigroups. We will focus on $\land$-prehomomorphisms, which are appropriate for the constructions of semidirect products.

<!-- 2.49 -->
<div class='remark' id='remark49'>
<span class='envidentifier'>Remark</span> <span class='counter'>2.49</span>
    The terminology <span>\&\#8220;</span>prehomomorphism<span>\&\#8221;</span> has been used to describe  $\lor$-prehomomorphisms in \cite[p.\ 80]{MR1694900}, but used to describe $\land$-prehomomorphisms in \cite[VI.7.2]{MR752899} and \cite{Mikola2017}. In order to avoid confusion, we will use the original terminology of <span>\&\#8220;</span>$\land$-prehomomorphism<span>\&\#8221;</span>.
</div>

A more specific case of $\land$-prehomomorphisms are <em>partial homomorphisms</em>, whose study was initiated by Exel in \cite{MR1276163}, in order to describe the structure of C*-algebras endowed with an action of the circle. This turned out to be a rich area of research, with several applications in the theory of topological dynamical systems and C*-algebras (see \cite{MR2799098,MR3699795}). Let us mention that partial homomorphisms of general inverse semigroups were first defined in \cite{MR3231479}, while partial actions of groupoids (on rings) were defined in \cite{MR2982887}.
...
<!-- 2.50 -->
<div class='definition' id='def:dualprehomomorphismandpartialhomomorphism'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.50</span>
...
<ol class='alt altlist9'>
\label{def:dualprehomomorphismandpartialhomomorphism1} For all $a\in\mathcal{S}$ we have $\theta(a^*)=\theta(a)^*$;
\label{def:dualprehomomorphismandpartialhomomorphism2} For all $a,b\in\mathcal{S}^{(2)}$, we have $(\theta(a),\theta(b))\in\mathcal{T}^{(2)}$ and $\theta(a)\theta(b)\leq\theta(ab)$;
\label{def:dualprehomomorphismandpartialhomomorphism3} If $a\leq b$ in $\mathcal{S}$, then $\theta(a)\leq\theta(b)$ in $\mathcal{T}$.
</ol>
If $\theta$ satisfies \ref{def:dualprehomomorphismandpartialhomomorphism1}-\ref{def:dualprehomomorphismandpartialhomomorphism2}, then $\theta$ is called a <em>$\land$-prehomomorphism</em>. If $\theta$ satisfies all of \ref{def:dualprehomomorphismandpartialhomomorphism1}-\ref{def:dualprehomomorphismandpartialhomomorphism3}, then $\theta$ is called a <em>partial homomorphism</em>.
</div>.

<!-- 2.51 -->
<div class='example' id='example51'>
<span class='envidentifier'>Example</span> <span class='counter'>2.51</span>
...
<!-- 2.52 -->
<div class='example' id='example52'>
<span class='envidentifier'>Example</span> <span class='counter'>2.52</span>
Let $G=\left\{1,g\right\}$ be the cyclic group of order $2$ and $X=\left\{a,b\right\}$. Define $\theta\colon G\to\mathcal{I}(X)$ as $\theta_1=\id_X$ and <div class='equation'>
\begin{equation*}
...
<!-- 2.53 -->
<div class='example' id='example53'>
<span class='envidentifier'>Example</span> <span class='counter'>2.53</span>
...
<!-- 2.54 -->
<div class='example' id='ex:landprehomomorphismnotpartialaction'>
<span class='envidentifier'>Example</span> <span class='counter'>2.54</span>
...
<!-- 2.55 -->
<div class='example' id='example55'>
<span class='envidentifier'>Example</span> <span class='counter'>2.55</span>
...
<!-- 2.56 -->
<div class='definition' id='def:partialactiononset'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.56</span>
...
<div class='equation' id='eq:changepreactionbygreater'>
\begin{equation*}
\theta_b(x)=\theta_b(\theta_{a^*a}(x))=\theta_{ba^*a}(x)=\theta_a(x).\tag{2.57}
\end{equation*}
</div>
...
<!-- 2.58 -->
<div class='example' id='ex:actionofsemigroup'>
<span class='envidentifier'>Example</span> <span class='counter'>2.58</span>
...
<!-- 2.59 -->
<div class='example' id='ex:groupoidnondegenerateaction'>
<span class='envidentifier'>Example</span> <span class='counter'>2.59</span>
...
<!-- 2.60 -->
<div class='definition' id='definition60'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.60</span>
...
<!-- 2.61 -->
<div class='example' id='example61'>
<span class='envidentifier'>Example</span> <span class='counter'>2.61</span>
...
<!-- 2.62 -->
<div class='definition' id='def:partialactiononsemigroupoid'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.62</span>
...
<ol class='alt altlist10'>
        \label{def:partialactiononsemigroupoid1} The anchor map $\pi\colon\Lambda\to\mathcal{S}^{(0)}$ is a homomorphism.
        \label{def:partialactiononsemigroupoid2} For every $x\in\mathcal{S}^{(0)}$, $\pi^{-1}(x)$ is an ideal of $\Lambda$;
        \label{def:partialactiononsemigroupoid3} For every $a\in\Lambda$, $\dom(\theta_a)$ is an ideal of $\pi^{-1}(\so(a))$;
        \label{def:partialactiononsemigroupoid4} For every $a\in\mathcal{S}$, $\theta_a$ is a semigroupoid isomorphism from $\dom(\theta_a)$ to $\ran(\theta_a)$.
    </ol>
</div>

<!-- 2.63 -->
<div class='remark' id='remark63'>
<span class='envidentifier'>Remark</span> <span class='counter'>2.63</span>
<ol>
     Alternatively to \ref{def:partialactiononsemigroupoid2} and \ref{def:partialactiononsemigroupoid3}, we could adopt the simpler (and slightly stronger) assumption that $\dom(\theta_a)$ is an ideal of $\Lambda$ for all $a\in\mathcal{S}$. This is the case, for example, when all such ideals $\dom(\theta_a)$ are <em>idempotent</em> (see Definition \ref{def:idempotentsemigroupoid}).
     If $\Lambda$ is a set/unit groupoid, then this definition coincides with \ref{def:partialactiononset}, since the additional properties of Definition \ref{def:partialactiononsemigroupoid} become trivial. Accordingly, by a $\land$-preaction we will always mean one in the sense of Definition \ref{def:partialactiononsemigroupoid}.
</ol>
</div>

As we will be more interested in <em>continuous $\land$-preactions of topological semigroupoids</em>, we refer to Subsection \ref{subsec:continuouslandpreactions} for examples. In fact, every $\land$-preaction $(\pi,\theta)\colon\mathcal{S}\curvearrowright\Lambda$ may be <span>\&\#8220;</span>extended<span>\&\#8221;</span> to a partial action, in the sense that partial action $(\pi,\overline{\theta})\colon\mathcal{S}\curvearrowright\Lambda$ (with same anchor map) such that $\theta_a\leq\overline{\theta}_a$ for all $a\in\mathcal{S}$. See Proposition \ref{prop:extensionoflandpreactiontopartialaction}

Let us finish this subsection by proving that any $\land$-preaction may be <span>\&\#8220;</span>extended<span>\&\#8221;</span> to a partial action.

<!-- 2.64 -->
<div class='proposition' id='prop:extensionoflandpreactiontopartialaction'>
<span class='envidentifier'>Proposition</span> <span class='counter'>2.64</span>
...
<!-- 2.65 -->
<div class='proof' id='proof65'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.65</span>
    As the only (possible) problem with is that $\theta$ does not necessarily satisfy $\theta_b\leq\theta_a$ when $b\leq a$, we simply <span>\&\#8220;</span>glue<span>\&\#8221;</span> $\theta_b$ to $\theta_a$ in this case.
...
<h3 class='subsection'>2.5. Semidirect products</h2>
...
<div class='equation' id='eq:semidirectproduct'>
\begin{equation*}
(a,x)(b,y)=(ab,\theta_{b^*}(x\theta_b(y))).\tag{2.66}
\end{equation*}
</div>
...
<!-- 2.67 -->
<div class='example' id='example67'>
<span class='envidentifier'>Example</span> <span class='counter'>2.67</span>
...
<!-- 2.68 -->
<div class='definition' id='definition68'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.68</span>
	We call $\mathcal{S}\ltimes\Lambda$ with the product \eqref{eq:semidirectproduct} the <em>semidirect product semigroupoid</em> (induced by the $\land$-preaction $(\pi,\theta)$), whenever the product \eqref{eq:semidirectproduct} is associative.
</div>
...
<!-- 2.69 -->
<div class='definition' id='definition69'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.69</span>
A <em>multiplier</em> of a semigroupoid $\Lambda$ is a pair $(L,R)$, of partially defined maps $L$ and $R$ on $\Lambda$ satisfying
<style>ol.altlist11{
content:  counter(listcounter,lower-roman);
}
</style>
<ol class='alt altlist11'>
...
<!-- 2.70 -->
<div class='example' id='ex:multipliersofmultiplication'>
<span class='envidentifier'>Example</span> <span class='counter'>2.70</span>
...
<!-- 2.71 -->
<div class='definition' id='definition71'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.71</span>
...
<!-- 2.72 -->
<div class='definition' id='definition72'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.72</span>
...
<!-- 2.73 -->
<div class='theorem' id='thm:semidirectproductisassociativeifLRassociative'>
<span class='envidentifier'>Theorem</span> <span class='counter'>2.73</span>
The product of $\mathcal{S}\ltimes\Lambda$ is associative if $\dom(\theta_a)$ is $(L,R)$-associative for all $a\in\mathcal{S}$.
</div>
<!-- 2.74 -->
<div class='proof' id='proof74'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.74</span>
...
<div class='equation' id='eq:equationwhichdeterminesassociativity1'>
\begin{equation*}
\theta_{c}^{-1}(\theta_{b}^{-1}(r\theta_{b}(s))w)=\theta_{bc}^{-1}(r\theta_b(sw))\tag{2.75}.
\end{equation*}
</div>

To further simplify the expression above we would need to rewrite the term <span>\&\#8220;</span>$\theta_{bc}^{-1}$<span>\&\#8221;</span>, at the beggining of the right-hand side above, as <span>\&\#8220;</span>$\theta_c^{-1}\circ\theta_b^{-1}$<span>\&\#8221;</span>, however this is not so immediate. If this is the case, Equation \eqref{eq:equationwhichdeterminesassociativity1} implies
<div class='equation' id='eq:equationwhichdeterminesassociativity2'>
\begin{equation*}
\theta_{b}^{-1}(r\theta_{b}(s))w=\theta_{b}^{-1}(r\theta_{b}(sw)).\tag{2.76}
\end{equation*}
</div>
...
<div class='equation' id='eq:prop:semidirectproductisassociativeifLRassociative'>
\begin{equation*}
R_{w}\circ \theta_{b}^{-1}\circ L_{r}\circ\theta_{b}=\theta_{b}^{-1}\circ L_{r}\circ \theta_{b}\circ R_{w}\tag{2.77}
\end{equation*}
</div>
...
<!-- 2.78 -->
<div class='proof' id='proof78'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.78</span> <span class='envop'>(Proof of the equivalence of Equations \eqref{eq:equationwhichdeterminesassociativity1} and \eqref{eq:equationwhichdeterminesassociativity2})</span>
...
<!-- 2.79 -->
<div class='definition' id='def:idempotentsemigroupoid'>
<span class='envidentifier'>Definition</span> <span class='counter'>2.79</span>
...
<!-- 2.80 -->
<div class='proposition' id='prop:idempotentornondegenerateimpliesLRassociative'>
<span class='envidentifier'>Proposition</span> <span class='counter'>2.80</span>
A semigroupoid $\Lambda$ is $(L,R)$-associative if any of the following two conditions below holds.
<style>ol.altlist12{
content:  counter(listcounter,lower-roman);
}
</style>
<ol class='alt altlist12'>
	\label{prop:idempotentornondegenerateimpliesLRassociative1} $\Lambda$ is idempotent;
	\label{prop:idempotentornondegenerateimpliesLRassociative2} $\Lambda$ is non-degenerate, and for every $a\in\Lambda$ we have $\Lambda_a\neq\varnothing$.
	\label{prop:idempotentornondegenerateimpliesLRassociative3} $\Lambda$ is non-degenerate, and for every $a\in\Lambda$ we have $\Lambda^a\neq\varnothing$.
</ol>
</div>
<!-- 2.81 -->
<div class='proof' id='proof81'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.81</span>
...
<!-- 2.82 -->
<div class='proposition' id='prop:semidirectproductisinverseiffsemigroupoidisinverse'>
<span class='envidentifier'>Proposition</span> <span class='counter'>2.82</span>
...
<!-- 2.83 -->
<div class='proof' id='proof83'>
<span class='envidentifier'>Proof</span> <span class='counter'>2.83</span>
...
<div class='equation' id='eq:prop:semidirectproductisinverseiffsemigroupoidisinverse'>
\begin{equation*}
(a,x)=(a,x\theta_{a^*}(z)x)\qquad\text{and}\qquad (a^*,z)=(a^*,z\theta_a(x)z).\tag{2.84}
\end{equation*}
</div>
...
<h2 class='section'>3. Topological and étale inverse semigroupoids</h2>
    We will now consider semigroupoid endowed with topologies which are compatible with their algebraic structures. In general, a (partial) algebraic structure $\mathscr{A}$ consists of families $\mathscr{A}_0$ of sets and $\mathscr{A}_1$ of partial functions between sets in $\mathscr{A}_0$, and $\mathscr{A}$ is said to be <em>topological</em> if all sets in $\mathscr{A}_0$ are endowed with topologies, and all functions in $\mathscr{A}_1$ are continuous. The homomorphisms between such structure are also assumed to be continuous. This applies, in particular, to graphs and (Exel/graphed/inverse) semigroupoids. We write the following definitions in full for the sake of completeness.

<!-- 3.85 -->
<div class='definition' id='definition85'>
<span class='envidentifier'>Definition</span> <span class='counter'>3.85</span>
A <em>topological graph</em> is a graph $(G^{(0)},G,\so,\ra)$, such that $G^{(0)}$ and $G$ are endowed with topologies making $\so$ and $\ra$ continuous.

A <em>topological</em> (Exel) semigroupoid is a semigroupoid $\Lambda$ endowed with some topology which makes the product continuous (where $\Lambda^{[2]}$ is endowed with the product topology of $\Lambda\times\Lambda$).
...
<!-- 3.86 -->
<div class='definition' id='definition86'>
<span class='envidentifier'>Definition</span> <span class='counter'>3.86</span>
A <em>topological inverse semigroupoid</em> is an inverse semigroupoid $\mathcal{S}$ which is a topological graphed semigroupoid, and such that the inverse map $(\ )^*\colon\mathcal{S}\to\mathcal{S}$, $s\mapsto s^*$, is continuous.
</div>

Note that since $(\ )^*\circ (\ )^*=\id_{\mathcal{S}}$, then $(\ )^*$ being continuous implies that it is a homeomorphism.

<!-- 3.87 -->
<div class='definition' id='def:etaleinversesemigroupoid'>
<span class='envidentifier'>Definition</span> <span class='counter'>3.87</span>
A topological inverse semigroupoid $\mathcal{S}$ is <em>étale</em> if the source map $\so\colon\mathcal{S}\to\mathcal{S}^{(0)}$ is a local homeomorphism. (Equivalently, the range map $\ra=\so\circ(\ )^*$ is a local homeomorphism.)
</div>

<!-- 3.88 -->
<div class='remark' id='remark88'>
<span class='envidentifier'>Remark</span> <span class='counter'>3.88</span>
As in \cite{MR2304314}, recall that a topological groupoid $\mathcal{G}$ is <em>étale</em> if the source map $\so\colon a\mapsto a^{-1}a$ is a local homeomorphism from $\mathcal{G}$ to $\mathcal{G}^{(0)}=\so(\mathcal{G})$, where <em>$\mathcal{G}^{(0)}$ is endowed with the subspace topology of $\mathcal{G}$</em>. It is not immediately clear that this coincides with the notion in Definition \ref{def:etaleinversesemigroupoid} since for étale semigroupoids we do not assume $\mathcal{S}^{(0)}$ to even be a subset of $\mathcal{S}$.
...
<!-- 3.89 -->
<div class='corollary' id='corollary89'>
<span class='envidentifier'>Corollary</span> <span class='counter'>3.89</span>
...
A <em>bisection</em> of a graphed semigroupoid $\mathcal{S}$ is a subset $U\subseteq\mathcal{S}$ such that the source and range maps are injective on $U$. We denote by $\mathbf{B}(\mathcal{S})$ the set of all open bisections of an étale inverse semigroupoid $\mathcal{S}$. If $U\in\mathbf{B}(\mathcal{S})$, then the source and range maps restrict to homeomorphisms from $U$ onto open subsets of $\mathcal{S}^{(0)}$. Moreover, $\mathbf{B}(\mathcal{S})$ is a basis for the topology of $\mathcal{S}$.

<!-- 3.90 -->
<div class='proposition' id='prop:productmapisopen'>
<span class='envidentifier'>Proposition</span> <span class='counter'>3.90</span>
If $\mathcal{S}$ is an étale inverse semigroupoid then the product map $\mu\colon\mathcal{S}^{(2)}\to\mathcal{S}$ is open.
</div>
<!-- 3.91 -->
<div class='proof' id='proof91'>
<span class='envidentifier'>Proof</span> <span class='counter'>3.91</span>
...
<!-- 3.92 -->
<div class='corollary' id='cor:propertiesopeninversesemigroupoid'>
<span class='envidentifier'>Corollary</span> <span class='counter'>3.92</span>
Let $\mathcal{S}$ be an étale inverse semigroupoid. Then
<style>ol.altlist13{
content:  counter(listcounter,lower-alpha);
}
</style>
<ol class='alt altlist13'>
    \label{cor:propertiesopeninversesemigroupoid1} $E(\mathcal{S})$ is open in  $\mathcal{S}$;
    \label{cor:propertiesopeninversesemigroupoid2} For all open $A\subseteq \mathcal{S}$, the <em>upper</em> and <em>lower closures</em> <div class='equation'>
\begin{equation*}
A^{\uparrow,\leq}=\left\{b\in\mathcal{S}:b\geq a\text{ for some }a\in A\right\}\quad\text{and}\quad A^{\downarrow,\leq}=\left\{b\in\mathcal{S}:b\leq a\text{ for some }a\in A\right\}
\end{equation*}
</div>
...
<!-- 3.93 -->
<div class='proof' id='proof93'>
<span class='envidentifier'>Proof</span> <span class='counter'>3.93</span>
<style>ol.altlist14{
content:  counter(listcounter);
}
</style>
<ol class='alt altlist14'>
     Simply note that $E(\mathcal{S})=\bigcup\left\{A^*A:A\in \mathbf{B}(\mathcal{S})\right\}$.
     The lower set $A^{\downarrow,\leq}=AE(\mathcal{S})$ is the product of open sets, hence open.
...
<!-- 3.94 -->
<div class='example' id='example94'>
<span class='envidentifier'>Example</span> <span class='counter'>3.94</span>
...
<!-- 3.95 -->
<div class='theorem' id='theorem95'>
<span class='envidentifier'>Theorem</span> <span class='counter'>3.95</span>
    Let $\mathcal{S}$ be a topological inverse semigroupoid. Then the following are equivalent:
    <style>ol.altlist15{
content:  counter(listcounter);
}
</style>
<ol class='alt altlist15'>
         $\mathcal{S}$ is étale;
         $E(\mathcal{S})$ is open and étale, and the product of open sets is open.
         $E(\mathcal{S})$ is open and étale, and $\mathcal{S}A$ is open for each open $A\subseteq\mathcal{S}$.
         $E(\mathcal{S})$ is open and étale, and $A^*A$ is open for each open $A\subseteq\mathcal{S}$.
         $E(\mathcal{S})$ is open and étale, and the source map is open;
    </ol>
</div>

<h3 class='subsection' id='subsec:continuouslandpreactions'>3.1. Continuous \texorpdfstring{$\land$}{∧}-preactions and semidirect products</h2>

Tipically, in the topological setting we consider actions which preserve the topological structure. Let us consider the <span>\&\#8220;</span>action map<span>\&\#8221;</span> associated to a $\land$-preaction $(\pi,\theta)\colon\mathcal{S}\curvearrowright\Lambda$, which we also denote by $\theta$:
<div class='equation'>
\begin{equation*}
...
<!-- 3.96 -->
<div class='definition' id='definition96'>
<span class='envidentifier'>Definition</span> <span class='counter'>3.96</span>
    A $\land$-preaction $(\pi,\theta)\colon\mathcal{S}\curvearrowright\Lambda$ of a topological inverse semigroupoid $\mathcal{S}$ on a topological semigroupoid $\Lambda$ is <em>continuous</em> if $\pi$ and the <span>\&\#8220;</span>action map<span>\&\#8221;</span> $\theta\colon\mathcal{S}\ltimes\Lambda\to\Lambda$ are continuous. If the action map $\theta$ is open then we call $(\pi,\theta)$ an <em>open</em> $\land$-preaction.
</div>
...
<!-- 3.97 -->
<div class='remark' id='remark97'>
<span class='envidentifier'>Remark</span> <span class='counter'>3.97</span>
Suppose that $(\pi,\theta)\colon\mathcal{S}\curvearrowright\Lambda$ is a continuous open $\land$-preaction. Given $a\in\mathcal{S}$, let $A$ be any open bisection containing $a$. Then <div class='equation'>
\begin{equation*}
\ran(\theta_a)=\pi^{-1}(\ra(a))\cap\theta(A\ast\Lambda)
\end{equation*}
</div>
...
<!-- 3.98 -->
<div class='proposition' id='prop:preactionisopeniffprojectionisopen'>
<span class='envidentifier'>Proposition</span> <span class='counter'>3.98</span>
...
<!-- 3.99 -->
<div class='proof' id='proof99'>
<span class='envidentifier'>Proof</span> <span class='counter'>3.99</span>
...
Conversely, assume that $p$ is open. The map <div class='equation'>
\begin{equation*}
I\colon\mathcal{S}\ltimes\Lambda\to\mathcal{S}\ltimes\Lambda,\qquad (a,x)\mapsto (a^*,\theta_a(x))
\end{equation*}
</div>
...
<!-- 3.100 -->
<div class='example' id='ex:munn'>
<span class='envidentifier'>Example</span> <span class='counter'>3.100</span>
    We will now describe an analogue, in the setting of semigroupoids, of the canonical actions of a semigroup on its idempotent semilattice (the <em>Munn representation</em>), and of 
...
<ol class='alt altlist16'>
         $\so(b)=\ra(b)$;
         For all $e\leq b^*b$, we have $beb^*=e$;
    </ol>
    ...
    <!-- 3.101 -->
<div class='itemize' id='itemize101'>
<span class='envidentifier'>Itemize</span> <span class='counter'>3.101</span>
         If $\mathcal{S}$ is an étale groupoid, then $\operatorname{F}(\mathcal{S})$ is the isotropy subgroupoid of $\mathcal{S}$. The restriction of $(\pi,\mu)$ to $E(\mathcal{S})=\mathcal{S}^{(0)}$ is the canonical action of $\mathcal{S}$ on its unit space: $\mu_g$ is defined only as $\mu_g(\so(g))=\ra(g)$, for each $g\in\mathcal{S}$.
         if $\mathcal{S}$ is a discrete inverse semigroup then the restriction of $(\pi,\mu)$ to $E(\mathcal{S})$ is the <em>Munn representation</em> of $\mathcal{S}$.
    </div>
</div>
...
<!-- 3.102 -->
<div class='example' id='ex:semidirectproductofactionisetale'>
<span class='envidentifier'>Example</span> <span class='counter'>3.102</span>
...
<!-- 3.103 -->
<div class='example' id='example103'>
<span class='envidentifier'>Example</span> <span class='counter'>3.103</span>
...
<!-- 3.104 -->
<div class='proposition' id='proposition104'>
<span class='envidentifier'>Proposition</span> <span class='counter'>3.104</span>
...
<!-- 3.105 -->
<div class='proof' id='proof105'>
<span class='envidentifier'>Proof</span> <span class='counter'>3.105</span>
    ...
    Similarly, if $A\subseteq\mathcal{S}$ and $V\subseteq\Lambda$ are open, then $\overline{\theta}(A\overline{\ast} V)=\theta(A^{\downarrow,\leq}\ast V)$, so $(\pi,\theta)$ being open implies that $(\pi,\overline{\theta})$ is also open. </div>

<h2 class='section' id='sec:categoricalconstructions'>4. Constructions with inverse semigroupoids</h2>
...
<h3 class='subsection'>4.1. Underlying groupoids</h2>
<!-- 4.106 -->
<div class='definition' id='definition106'>
<span class='envidentifier'>Definition</span> <span class='counter'>4.106</span>
The <em>underlying</em> or <em>restricted product groupoid</em> of a semigroupoid $\mathcal{S}$ is the groupoid $\mathscr{U}(\mathcal{S})$ obtained from $\mathcal{S}$ by restricting its product to the set $\mathscr{U}(\mathcal{S})^{(2)}=\left\{(a,b)\in \mathcal{S}\times \mathcal{S}:a^*a=bb^*\right\}$.
</div>
...
<!-- 4.107 -->
<div class='proposition' id='proposition107'>
<span class='envidentifier'>Proposition</span> <span class='counter'>4.107</span>
...
<h3 class='subsection'>4.2. Quotients</h2>
...
<!-- 4.108 -->
<div class='definition' id='definition108'>
<span class='envidentifier'>Definition</span> <span class='counter'>4.108</span>
If $G$ is a graph, an equivalence relation $R$ on $G^{(1)}$ is said to be <em>graphed</em> the source and range maps are $R$-invariant (i.e., constant on all $R$-equivalence classes).

A <em>graph congruence</em> on an inverse semigroupoid $\mathcal{S}$ is a graphed equivalence relation $R$ on $\mathcal{S}$ such that for all $a,\widetilde{a},b,\widetilde{b}\in \mathcal{S}$, if $(a,\widetilde{a}),(b,\widetilde{b})\in R$ and $(a,b)\in\mathcal{S}^{(2)}$, then $(ab,\widetilde{a}\widetilde{b})\in R$.
</div>
...
<!-- 4.109 -->
<div class='lemma' id='lem:regularisinverseiffidempotentscommute'>
<span class='envidentifier'>Lemma</span> <span class='counter'>4.109</span> <span class='envop'>({\cite[Lemma 3.3.1]{MR3597709}})</span>
...
<!-- 4.110 -->
<div class='proposition' id='proposition110'>
<span class='envidentifier'>Proposition</span> <span class='counter'>4.110</span>
...
Suppose now that $\mathcal{S}$ is a topological (or étale) inverse semigroupoid, and that $R$ is a graphed congruence on $\mathcal{S}$. We wish to endow $\mathcal{S}/R$ with a natural topology making it a topological (or étale) inverse semigroupoid as well, and for this we use the <em>quotient topology</em>. However, in general we do not have control over the topology of $(\mathcal{S}/R)^{(2)}$, and thus we cannot guarantee that the multiplication map of $\mathcal{S}/R$ is continuous with respect to the quotient topology. We will therefore to make further topological assumptions on the congruence $R$ in order to obtain the desired result. Let us briefly recall some facts about quotient topologies.

<!-- 4.111 -->
<div class='itemize' id='itemize111'>
<span class='envidentifier'>Itemize</span> <span class='counter'>4.111</span>
     If $X$ is a topological space, $Y$ is a set, and $\pi\colon X\to Y$ is a function, the <em>quotient topology</em> of $Y$ (induced by $\pi$) has as open subsets the subsets $A$ of $Y$ such that $\pi^{-1}(A)$ is open in $X$. In this case, a function $p\colon Y\to Z$, where $Z$ is a topological space, is continuous if and only if $p\circ\pi$ is continuous. In other words, continuous maps from $Y$ are precisely the factors of continuous maps from $X$ through $\pi$.
...
Suppose that $R$ is an equivalence relation on a topological space $X$. Given a subset $A\subseteq\mathcal{S}$, we let $R[A]$ denote the <em>saturation</em> of $A$, i.e., $R[A]\defeq\left\{x\in X:xRa\text{ for some }a\in A\right\}$. 
We say that $R$ is <em>open</em> if the saturation of every open subset of $X$ is open, or equivalently if the quotient map $X\to X/R$ is an open map.

<!-- 4.112 -->
<div class='proposition' id='prop:equivalecesetalequotient'>
<span class='envidentifier'>Proposition</span> <span class='counter'>4.112</span>
...
<!-- 4.113 -->
<div class='proof' id='proof113'>
<span class='envidentifier'>Proof</span> <span class='counter'>4.113</span>
...
<!-- 4.114 -->
<div class='remark' id='remark114'>
<span class='envidentifier'>Remark</span> <span class='counter'>4.114</span>
...
<h3 class='subsection' id='subsec:groupoidofgerms'>4.3. Semigroupoids of germs and the initial groupoid</h2>

<!-- 4.115 -->
<div class='definition' id='definition115'>
<span class='envidentifier'>Definition</span> <span class='counter'>4.115</span>
A poset $(P,\preceq)$ is <em>conditionally downwards directed</em> for all $p\leq P$, the downset $p^{\downarrow,\preceq}=\left\{x\in P:x\preceq p\right\}$ is downwards directed. Explicitly, this means that whenever $x,y\preceq p$ in $P$, there exists $z\in P$ with $z\preceq x,y$.
</div>

As an example, every inverse semigroupoid with its canonical order is conditionally downwards directed.

The <em>germ relation</em> $\sim_{\preceq}$ on a poset $(P,\preceq)$ is defined as
<div class='equation'>
\begin{equation*}
...
<!-- 4.116 -->
<div class='lemma' id='lemma116'>
<span class='envidentifier'>Lemma</span> <span class='counter'>4.116</span>
...
<!-- 4.117 -->
<div class='proof' id='proof117'>
<span class='envidentifier'>Proof</span> <span class='counter'>4.117</span>
...
<!-- 4.118 -->
<div class='definition' id='def:compatibleorder'>
<span class='envidentifier'>Definition</span> <span class='counter'>4.118</span>
    A preorder $\preceq$ on a semigroupoid $\mathcal{S}$ is <em>compatible</em> (with $\mathcal{S}$) if
    <style>ol.altlist17{
content:  counter(listcounter,lower-roman);
}
</style>
<ol class='alt altlist17'>
        \label{def:compatibleorder1} $a\preceq b$ implies $a\leq b$;
        \label{def:compatibleorder2} $a\preceq b$ implies $ax\preceq bx$ and $ya\preceq yb$ whenever $(a,x),(y,a)\in\mathcal{S}^{(2)}$.
    </ol>
...
<!-- 4.119 -->
<div class='example' id='ex:setorderofbisections'>
<span class='envidentifier'>Example</span> <span class='counter'>4.119</span>.
...
<!-- 4.120 -->
<div class='lemma' id='lem:relation.of.germs.is.a.graphed.congruence'>
<span class='envidentifier'>Lemma</span> <span class='counter'>4.120</span>
...
<!-- 4.121 -->
<div class='proof' id='proof121'>
<span class='envidentifier'>Proof</span> <span class='counter'>4.121</span>
...
<!-- 4.122 -->
<div class='remark' id='remark122'>
<span class='envidentifier'>Remark</span> <span class='counter'>4.122</span>
...
<!-- 4.123 -->
<div class='definition' id='definition123'>
<span class='envidentifier'>Definition</span> <span class='counter'>4.123</span>
A <em>topologically compatible</em> order $\preceq$ on a topological inverse semigroupoid $\mathcal{S}$ is a compatible order $\preceq$ such that upper and lower closures of open sets are also open, i.e., if $A\subseteq\mathcal{S}$ is open, then
<div class='equation'>
\begin{equation*}
...
<!-- 4.124 -->
<div class='example' id='example124'>
<span class='envidentifier'>Example</span> <span class='counter'>4.124</span>
...
<!-- 4.125 -->
<div class='proposition' id='proposition125'>
<span class='envidentifier'>Proposition</span> <span class='counter'>4.125</span>
...
<!-- 4.126 -->
<div class='definition' id='definition126'>
<span class='envidentifier'>Definition</span> <span class='counter'>4.126</span>
    The <em>initial groupoid</em> of an étale inverse semigroupoid is the topological (semi)groupoid $\IG(\mathcal{S})=\mathcal{S}/\!\!\sim_{\leq}$.
</div>
...
<!-- 4.127 -->
<div class='proposition' id='proposition127'>
<span class='envidentifier'>Proposition</span> <span class='counter'>4.127</span>
...
<!-- 4.128 -->
<div class='example' id='example128'>
<span class='envidentifier'>Example</span> <span class='counter'>4.128</span>
If $S$ is a discrete inverse semigroup, then $\IG(S)$ is called the <em>maximal group homomorphic image</em> of $S$. See \cite[Proposition 2.1.2]{MR1724106}.
</div>

<!-- 4.129 -->
<div class='example' id='example129'>
<span class='envidentifier'>Example</span> <span class='counter'>4.129</span>
If $\theta\colon S\curvearrowright X$ is a continuous $\land$-preaction of an inverse semigroup $S$ on a topological space $X$, then we have seen in Example \ref{ex:semidirectproductofactionisetale} the semidirect product $S\ltimes X$ is an étale inverse semigroupoid. The groupoid $\IG(S\ltimes X)$ is the <em>groupoid of germs</em> of $\theta$, as considered in \cite{arxiv1804.00396}.
</div>

\subsubsection*{Quotients of semidirect products}

Let $(\pi,\theta)\colon\mathcal{S}\curvearrowright\mathcal{T}$ be an open, continuous $\land$-preaction, where $\mathcal{S}$ and $\mathcal{T}$ are topological inverse semigroupoids. We will now analyse how the operations of <span>\&\#8220;</span>taking quotients<span>\&\#8221;</span> and <span>\&\#8220;</span>taking semidirect products<span>\&\#8221;</span> behave with respect to each other. Abusing language, we might simply say that <span>\&\#8220;</span>quotients and semidirect products commute<span>\&\#8221;</span>.
...
<!-- 4.130 -->
<div class='definition' id='definition130'>
<span class='envidentifier'>Definition</span> <span class='counter'>4.130</span>
    A graphed congruence $R$ on an inverse semigroupoid $R$ is <em>idempotent pure</em> if $(a,e)\in R$ and $e\in E(\mathcal{S})$ implies $a\in\mathcal{E(\mathcal{S})}$ (i.e., the saturation of $E(\mathcal{S})$ is $E(\mathcal{S})$).
</div>
...
<!-- 4.131 -->
<div class='proposition' id='prop:equivalencesidempotentpure'>
<span class='envidentifier'>Proposition</span> <span class='counter'>4.131</span>
The following are equivalent:
<style>ol.altlist18{
content:  counter(listcounter,lower-roman);
}
</style>
<ol class='alt altlist18'>
    \label{prop:equivalencesidempotentpure1} $R$ is idempotent pure;
    \label{prop:equivalencesidempotentpure2} The canonical quotient map $\pi\colon\mathcal{S}\to\mathcal{S}/R$ is idempotent pure, in the sense that $\pi^{-1}(E(\mathcal{S}/R))=E(\mathcal{S})$.
    \label{prop:equivalencesidempotentpure3} If $(a,b)\in R$, then $a^*b\in E(\mathcal{S})$ (and also $ab^*\in E(\mathcal{S})$);
</ol>
</div>
...
<ol class='alt altlist19'>
\label{hyp:onrelationsforsemidirectproduct1} $R_1$ is idempotent pure;
\label{hyp:onrelationsforsemidirectproduct2} $\theta$ is an $R_2$-morphism: For all $a\in\mathcal{S}$ and $x,y\in\dom(\theta_a)$, if $(x,y)\in R_2$ then $(\theta_a(x),\theta_a(y))\in R_2$.
</ol>

<!-- 4.132 -->
<div class='lemma' id='lem:hypmakeformulasgood'>
<span class='envidentifier'>Lemma</span> <span class='counter'>4.132</span>
...
<!-- 4.133 -->
<div class='proof' id='proof133'>
<span class='envidentifier'>Proof</span> <span class='counter'>4.133</span>
...
We have already seen that if $a,b\in\alpha$ then $\theta_a$ and $\theta_b$ coincide on their common domain, and from this it follows that $\Theta_\alpha$ is a bijection from $D_\alpha$ to $R_\alpha$. We will prove that it is a $\land$-preaction, in the sense of Definition \ref{def:partialactiononsemigroupoid}. For this, let us make a small detour to the theory of <em>$\lor$-prehomomorphisms</em>, which are dual to $\land$-prehomomorphisms.

<!-- 4.134 -->
<div class='definition' id='definition134'>
<span class='envidentifier'>Definition</span> <span class='counter'>4.134</span>
A <em>$\lor$-prehomomorphism</em> between inverse semigroupoids is a map $\theta\colon\mathcal{S}\to\mathcal{T}$ which satisfies $(\theta\times\theta)(\mathcal{S}^{(2)})\subseteq\mathcal{T}^{(2)}$ and $\theta(ab)\leq\theta(a)\theta(b)$ whenever $(a,b)\in\mathcal{S}^{(2)}$.
</div>

<!-- 4.135 -->
<div class='proposition' id='prop:invertiblelorprehomarehom'>
<span class='envidentifier'>Proposition</span> <span class='counter'>4.135</span>
...
<!-- 4.136 -->
<div class='proof' id='proof136'>
<span class='envidentifier'>Proof</span> <span class='counter'>4.136</span>
...
<!-- 4.137 -->
<div class='proposition' id='proposition137'>
<span class='envidentifier'>Proposition</span> <span class='counter'>4.137</span>
If $(\pi,\theta)$ is a $\land$-preaction then $(\pi,\Theta)$ is a $\land$-preaction.
</div>
<!-- 4.138 -->
<div class='proof' id='proof138'>
<span class='envidentifier'>Proof</span> <span class='counter'>4.138</span>
...
<!-- 4.139 -->
<div class='remark' id='remark139'>
<span class='envidentifier'>Remark</span> <span class='counter'>4.139</span>
...
<!-- 4.140 -->
<div class='lemma' id='lem:topologicalpropertiesofacctionpasstoquotients'>
<span class='envidentifier'>Lemma</span> <span class='counter'>4.140</span>
...
<ol class='alt altlist20'>
    \label{lem:topologicalpropertiesofacctionpasstoquotients1} $\pi_1\times\pi_2\colon\mathcal{S}\ltimes\mathcal{S}\to(\mathcal{S}/R_1)\ltimes(\mathcal{T}/R_2)$ is surjective, continuous and open. In particular, $(\mathcal{S}/R_1)\ltimes(\mathcal{T}/R_2)$ carries the quotient topology of $\pi_1\times\pi_2$.
    \label{lem:topologicalpropertiesofacctionpasstoquotients2} $R_1\times R_2$ is open.
    \label{lem:topologicalpropertiesofacctionpasstoquotients3} $(\pi,\Theta)$ is open and continuous.
</ol>
</div>
<!-- 4.141 -->
<div class='proof' id='proof141'>
<span class='envidentifier'>Proof</span> <span class='counter'>4.141</span>
...
<!-- 4.143 -->
<div class='theorem' id='theorem143'>
<span class='envidentifier'>Theorem</span> <span class='counter'>4.143</span>
...
<!-- 4.144 -->
<div class='proof' id='proof144'>
<span class='envidentifier'>Proof</span> <span class='counter'>4.144</span>
...
<h3 class='subsection'>4.4. Global actions</h2>

In this subsection, let us consider global open actions of étale inverse semigroupoids on sets.

<!-- 4.145 -->
<div class='example' id='ex:actiononvertexspace'>
<span class='envidentifier'>Example</span> <span class='counter'>4.145</span>
...
<!-- 4.146 -->
<div class='proposition' id='proposition146'>
<span class='envidentifier'>Proposition</span> <span class='counter'>4.146</span>
$(\mathbb{T},\ast)$ is a pair of adjoint functors. Moreover, $\ast\mathbb{T}$ is (naturally isomorphic to) the identity of $\cat{EtIS}$.
</div>
<!-- 4.147 -->
<div class='proof' id='proof147'>
<span class='envidentifier'>Proof</span> <span class='counter'>4.147</span>
...
<h2 class='section' id='sec:duality'>5. Duality</h2>
...
<!-- 5.148 -->
<div class='lemma' id='lem:ES.Hausdorff.and.S0.Hausdorff'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.148</span>
...
<ol class='alt altlist22'>
        \label{lem:ES.Hausdorff.and.S0.Hausdorff1} If $E(\mathcal{S})$ is Hausdorff and , then $\mathcal{S}^{(0)}$ is Hausdorff as well.
        \label{lem:ES.Hausdorff.and.S0.Hausdorff2} If $\mathcal{S}^{(0)}$ is Hausdorff. then the product of compact subsets of $\mathcal{S}$ is compact.
    </ol>
</div>
<!-- 5.149 -->
<div class='proof' id='proof149'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.149</span>
    <style>ol.altlist23{
content: ;
}
</style>
<ol class='alt altlist23'>
     Suppose $E(\mathcal{S})$ is Hausdorff. Given $x_1\neq x_2$ in $\mathcal{S}^{(0)}$, choose $e_1,e_2\in E(\mathcal{S})$ with $\so(e_i)=x_i$, so $e_1\neq e_2$. Take any two open bisections $U_1,U_2$ with $e_i\in U_i$. 
     We have $\mathcal{S}^{(2)}=\left\{(a,b)\in\mathcal{S}\colon \so(a)=\ra(b)\right\}$, which is closed in $\mathcal{S}\times\mathcal{S}$ as $\so$ and $\ra$ are continuous and $\mathcal{S}^{(0)}$ is Hausdorff.
...
<!-- 5.150 -->
<div class='example' id='example150'>
<span class='envidentifier'>Example</span> <span class='counter'>5.150</span>
...
<!-- 5.151 -->
<div class='definition' id='definition151'>
<span class='envidentifier'>Definition</span> <span class='counter'>5.151</span>
    An étale inverse semigroupoid $\mathcal{S}$ is <em>ample</em> if $\mathcal{S}^{(0)}$ is locally compact, Hausdorff and zero-dimensional. We denote by $\mathbf{KB}(\mathcal{S})$ the inverse semigroup of compact-open bisections of $\mathcal{S}$.
</div>
...
<!-- 5.152 -->
<div class='definition' id='def:sigmaorder'>
<span class='envidentifier'>Definition</span> <span class='counter'>5.152</span>
A <em>$\Sigma$-ordered</em> inverse semigroup is an inverse semigroup with zero $S$ equipped with a compatible order $\subseteq$ satisfying
<style>ol.altlist24{
content:  counter(listcounter,lower-roman);
}
</style>
<ol class='alt altlist24'>
    \label{def:sigmaorder.0minimum} $0\subseteq a$ for all $a\in S$.
    \label{def:sigmaorder.conditionaljoins} $(S,\subseteq)$ admits <em>conditional joins</em>: If $a_1,a_2\subseteq c$, then the join $a_1\cup a_2=\sup_{\subseteq}\left\{a_1,a_2\right\}$ exists.
    \label{def:sigmaorder.distributivity} $(S,\subseteq)$ is (finitely) <em>distributive</em>: If a join $c_1\cup c_2$ exists and $a\in S$, then $a(c_1\cup c_2)=(ac_1)\cup (ac_2)$ (the latter term exists by the previous item).
    \label{def:sigmaorder.ESrelativecomplements} $(E(S),\subseteq)$ admits relative complements: If $e\subseteq f$ in $E(S)$, then there exists a $c\in E(S)$ such that $e\cup c=f$ and $e\cap c=0$. Such $c$ is necessarily unique (see next paragraph), and denoted by $f\setminus e$.
    \label{def:sigmaorder.zerojoins} If $a^*b=ab^*=0$, then the join $a\cup b$ exists.
    \label{def:sigmaorder.interpolation} If $t\leq a$ in $S$, then there exists $z\subseteq a$ such that $t\leq z\subseteq a$ and
    <div class='equation'>
\begin{equation*}
...
<!-- 5.153 -->
<div class='lemma' id='lem:technicalpropertiesofsigmaorder'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.153</span>
Suppose that $(S,\subseteq)$ is a $\Sigma$-ordered inverse semigroup.
    <style>ol.altlist25{
content:  counter(listcounter,lower-alpha);
}
</style>
<ol class='alt altlist25'>
        \label{lem:technicalpropertiesofsigmaorder1} If $x\leq y\leq z$ and $x\subseteq z$, then $x\subseteq y$.
        \label{lem:technicalpropertiesofsigmaorder2} If $a\cap b$ exists and $c\subseteq b$, then $a\cap c$ exists, and $a\cap c=(a\cap b)c^*c$.
        \label{lem:technicalpropertiesofsigmaorder3} If $x,y\subseteq a$, then $x\cap y$ exists, and $x\cap y=xy^*y=yy^*x$.
        \label{lem:technicalpropertiesofsigmaorder4} If $x,y\subseteq a$ and $b\in S$, then $b(x\cap y)=bx\cap by$.
        \label{lem:technicalpropertiesofsigmaorder5} If $a\cup b$ exists, then $(a\cup b)^*(a\cup b)=a^*a\cup b^*b$
        \label{lem:technicalpropertiesofsigmaorder6} If $b\cup c$ exists, then
        <div class='equation'>
\begin{equation*}
...
\end{equation*}
        ...
        \label{lem:technicalpropertiesofsigmaorder7} $S$ admits relative complements. More precisely, if $a\subseteq b$, then $b\setminus a=b(b^*b\setminus a^*a)$. (Note that the relative complement $b^*b\setminus a^*a$ is defined by \ref{def:sigmaorder.ESrelativecomplements}.)
        \label{lem:technicalpropertiesofsigmaorder8} If $a\cup b$, $a\cup c$ and $b\cap c$ are defined, then
        <div class='equation'>
\begin{equation*}
...
\end{equation*}
        ...
        \label{lem:technicalpropertiesofsigmaorder9} If $a\subseteq b$ and $c\in S$, then $c(b\setminus a)=(cb)\setminus (ca)$.
        \label{lem:technicalpropertiesofsigmaorder10} (De Morgan's Laws) If $a_1,a_2\subseteq b$ then $(b\setminus a_1)\cap(b\setminus a_2)=b\setminus(a_1\cup a_2)$ and $(b\setminus a_1)\cup(b\setminus a_2)=(b\setminus a_1\cap a_2)$.
    </ol>
</div>
<!-- 5.154 -->
<div class='proof' id='proof154'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.154</span>
    <style>ol.altlist26{
content:  counter(listcounter);
}
</style>
<ol class='alt altlist26'>
         As $x\subseteq z$, we multiply both sides by $y^*y$ to obtain $x=xy^*y\subseteq zy^*y=y$, because $x\leq y\leq z$.
         On one hand, $(a\cap b)c^*c\subseteq (a\cap b)b^*b=a\cap b\subseteq a$, and $(a\cap b)c^*c\subseteq bc^*c=c$, so $(a\cap b)c^*c$ is a $\subseteq$-lower bound of $\left\{a,c\right\}$. Simple computations prove that it is the greatest lower bound.
         This was already proven in Lemma \ref{lem:relation.of.germs.is.a.graphed.congruence}.
         As $bx,by\subseteq ba$, item \ref{lem:technicalpropertiesofsigmaorder2} yields $bx\cap by=bx(by)^*(by)$. As $x,y\leq a$, then $xy^*\in E(S)$, so we may commute $xy^*$ and $b^*b$ to obtain
        <div class='equation'>
\begin{equation*}
...
\end{equation*}
        ...
        where we used item \ref{lem:technicalpropertiesofsigmaorder2} in the last equality, as $x\cap y=xy^*y$.
         ...
         By distributivity, Property \ref{def:sigmaorder.distributivity},
        <div class='equation'>
\begin{equation*}
...
\end{equation*}
         ...
         If $a\cap(b\cup c)$ exists, then item \ref{lem:technicalpropertiesofsigmaorder5} implies that both $a\cap b$ and $a\cap c$ exist, Since both are $\subseteq a$, their $\subseteq$-join exists by Property \ref{def:sigmaorder.conditionaljoins}.
...
\end{equation*}
        ...
        that is, $p\leq(a\cap b)\cup(a\cap c)\leq b\cup c$. Since $p\subseteq b\cup c$, item \ref{lem:technicalpropertiesofsigmaorder1} gives us $p\subseteq (a\cap b)\cup(a\cap c)$, as we wanted.
         ...
         Suppose $a\subseteq b$. Let $e=b^*b\setminus a^*a$, i.e., $a^*a\cup e=b^*b$ and $a^*a\cap e=0$. Multiplying both equalities by $b$ on the left on both sides and using distributivity (item \ref{lem:technicalpropertiesofsigmaorder3} and Property \ref{def:sigmaorder.distributivity}) yields $a\cup(be)=b$ and $a\cap(be)=0$, so $be$ is the complement of $a$ relative to $b$.
        
         Since both $a$ and $b\cap c$ are $\subseteq$-bounded above by $a\cup b$, then $a\cup(b\cap c)$ is defined by Property \ref{def:sigmaorder.conditionaljoins}, and is a $\subseteq$-lower bound of $a\cup b$. Symmetrically, changing the roles of $b$ and $c$, we see that it is a $\subseteq$-lower bound of $\left\{a\cup b,a\cup c\right\}$. Again, let us prove that it is the largest one. Suppose $p\subseteq a\cup b,a\cup c$. Let us verify some cases:
        <style>ol.altlist27{
content:  counter(listcounter);
}
</style>
<ol class='alt altlist27'>
             If $p\subseteq a$, then of course $p\subseteq a\cup(b\cap c)$.
             Suppose $p\cap a=0$. Since both $p$ and $a$ are $\subseteq a\cup b$, then this means that $0=ap^*p$, so $p=(a\cup b)p^*p=ap^*p\cup bp^*p=bp^*p$, and thus $p\leq bp^*p$. Both $p$ and $b$ are $\subseteq a\cup b$, so $p\subseteq b$. Similarly, $p\subseteq c$, so $p\subseteq b\cap c\subseteq a\cup(b\cap c)$.
             For a general lower bound $p$ of $\left\{a\cup b,a\cup c\right\}$, we apply the first case to $p\cap a$ (which exists by item \ref{lem:technicalpropertiesofsigmaorder3}, since $a,p\subseteq a\cup b$) and the second one to $p\setminus(p\cap a)$, to conclude that
            <div class='equation'>
\begin{equation*}
...
\end{equation*}
        ...
        The last case proves that $a\cup(b\cap c)$ is the meet of $(a\cup b)\cap(a\cup c)$, as we wanted.
         If $a\subseteq b$ and $c\in S$, then it is easy enough, with the previous items, to verify that $c(b\setminus a)$ satisfies the properties of the complement of $ca$ in $cb$, so $c(b\setminus a)=(cb)\setminus(ca)$.
         Similarly to the previous item, it is only a matter of verifying that $(b\setminus a_1)\cap(b\setminus a_2)$ satisfies the defining properties of the complement $b\setminus (a_1\cup a_2)$, so these elements coincide, and similarly $b\setminus (a_1\cap a_2)=(b\setminus a_1)\cup(b\setminus a_2)$.\qedhere
    </ol>
</div>
...
<!-- 5.155 -->
<div class='lemma' id='lemma155'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.155</span>
...
<!-- 5.156 -->
<div class='proof' id='proof156'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.156</span>
...
<!-- 5.157 -->
<div class='definition' id='definition157'>
<span class='envidentifier'>Definition</span> <span class='counter'>5.157</span>
    The unique element $p$ of the Lemma above is called the <em>interpolator</em> of $t$ and $a$, and is denoted by $a|t$.
</div>

Interpolators may be described precisely in well-known cases.

<!-- 5.158 -->
<div class='example' id='example158'>
<span class='envidentifier'>Example</span> <span class='counter'>5.158</span>
...
<!-- 5.159 -->
<div class='example' id='ex:weaklybooleanaresigma'>
<span class='envidentifier'>Example</span> <span class='counter'>5.159</span>
...
<h3 class='subsection' id='subsec:reconstructionofvertexspace'>5.2. Reconstruction of \texorpdfstring{$\mathcal{S}^{(0)}$}{S0} from \texorpdfstring{$\mathbf{KB}(\mathcal{S})$}{KB(S)}</h2>

Let $E$ be a semilattice. Recall that a <em>filter</em> on $E$ is a nonempty subset $\mathfrak{F}\subseteq E$ which is <em>downwards directed</em> and <em>upwards closed</em> - i.e., it satisfies
<style>ol.altlist28{
content:  counter(listcounter,lower-roman);
}
</style>
<ol class='alt altlist28'>
     If $e,f\in\mathfrak{F}$, then $ef\in\mathfrak{F}$;
     If $e\in\mathfrak{F}$, $f\in E$, and $e\leq f$, then $f\in\mathfrak{F}$.
</ol>

A filter is <em>proper</em> if $\mathfrak{F}\neq E$. If $E$ has a zero (minimum), this is equivalent to say that $0\not\in\mathfrak{F}$. An <em>ultrafilter</em> is a maximal proper filter. Every nonzero element $e\in E$ belongs to the filter $e^\uparrow=\left\{f\in E:e\leq f\right\}$, and Zorn's Lemma implies that every filter is contained in some ultrafilter. Therefore every element of $E$ belongs to some ultrafilter.

The following alternative description of ultrafilters is well-known and useful.

<!-- 5.160 -->
<div class='lemma' id='lem:description.of.ultrafilters.by.nonzero.products'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.160</span>
    Let $\mathfrak{F}$ be a proper filter in a semilattice with zero $E$. The following are equivalent:
    <style>ol.altlist29{
content:  counter(listcounter);
}
</style>
<ol class='alt altlist29'>
        \label{lem:description.of.ultrafilters.by.nonzero.products.1} $\mathfrak{F}$ is an ultrafilter;
        \label{lem:description.of.ultrafilters.by.nonzero.products.2} For every $e\in E$, if $0\not\in e\mathfrak{F}$ then $e\in\mathfrak{F}$.
    </ol>
</div>
<!-- 5.161 -->
<div class='proof' id='proof161'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.161</span>
...
<!-- 5.162 -->
<div class='definition' id='definition162'>
<span class='envidentifier'>Definition</span> <span class='counter'>5.162</span>
    The <em>spectrum</em> of $E$ is the topological space $\Omega(E)$ of all ultrafilters in $E$, with the topology generated by the sets
    <div class='equation'>
\begin{equation*}
...
Suppose now that $(S,\subseteq)$ is a $\Sigma$-ordered inverse semigroup, and $E=E(S)$ is its idempotent semilattice (we use the <em>canonical order</em> $\leq$ as the lattice structure of $E(S)$, and not $\subseteq$).

<!-- 5.163 -->
<div class='lemma' id='lem:ultrafilters.are.prime'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.163</span>
...
<!-- 5.164 -->
<div class='proof' id='proof164'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.164</span>
...
<!-- 5.165 -->
<div class='proposition' id='proposition165'>
<span class='envidentifier'>Proposition</span> <span class='counter'>5.165</span>
$\Omega(E(S))$ is a zero-dimensional, locally compact Hausdorff space.
</div>
<!-- 5.166 -->
<div class='proof' id='proof166'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.166</span>
...
Suppose now that $\mathcal{S}$ is an ample inverse semigroupoid. For every $x\in\mathcal{S}^{(0)}$, the set <div class='equation'>
\begin{equation*}
\psi(x)\defeq\left\{U\in E(\mathbf{KB}(\mathcal{S})):x\in\so(U)\right\}
\end{equation*}
</div>
is clearly a proper filter in $E(\mathbf{KB}(\mathcal{S}))$.

<!-- 5.167 -->
<div class='lemma' id='lem:homeomorphisms0spectrum'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.167</span>
...
<!-- 5.168 -->
<div class='proof' id='proof168'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.168</span>
...
<h3 class='subsection'>5.3. The category \texorpdfstring{$\Sigma\cat{-Ord}$}{Σ-Ord}</h2> A <em>morphism</em> of $\Sigma$-ordered inverse semigroups is a semigroup homomorphism $\theta\colon S\to T$ which further satisifies:
<style>ol.altlist30{
content:  counter(listcounter,lower-roman);
}
</style>
<ol class='alt altlist30'>
    \label{def:morphismofsigmaordered.zero} $\theta(0)=0$;
    \label{def:morphismofsigmaordered.monotone} $\theta$ is $\subseteq$-monotone: if $a\subseteq b$ in $S$ then $\phi(a)\subseteq\phi(b)$ in $T$;
    \label{def:morphismofsigmaordered.cup} $\theta$ is a $\cup$-morphism: if $a\cup b$ exists in $S$, then $\theta(a)\cup\theta(b)$ exists in $T$, and $\theta(a\cup b)=\theta(a)\cup\theta(b)$;
    \label{def:morphismofsigmaordered.weaklymeet} For all $a,b\in S$ and $t\in T$, if $t\subseteq \theta(a),\theta(b)$, then there exists $c\subseteq a,b$ such that $t\subseteq\theta(c)$;
    \label{def:morphismofsigmaordered.proper} $\theta$ is <em>proper</em>: For all $t\in T$, there exist $t_1,\ldots,t_n\in T$ and $s_1,\ldots,s_n\in S$ such that
    <div class='equation'>
\begin{equation*}
t=\bigcup_{i=1}^n t_i\qquad\text{and}\qquad t_i\subseteq\theta(s_i)\text{ for all }i=1,\ldots,n.
\end{equation*}
</div>
    \label{def:morphismofsigmaordered.interpolators} $\theta$ preserves interpolators: If $a\leq b$, then $\theta(b|a)=\theta(b)|\theta(a)$.
</ol>
...
Property \ref{def:morphismofsigmaordered.weaklymeet} above was considered in \cite[p.\ 134]{MR3077869}, as part of the definition of <em>callitic morphisms</em> for distributive inverse semigroups. It is a strengthening of the condition that $\theta$ preserves $\land$, and is necessary since we consider non-Hausdorff inverse semigroupoids.
...
<!-- 5.169 -->
<div class='example' id='example169'>
<span class='envidentifier'>Example</span> <span class='counter'>5.169</span>
...
<h3 class='subsection'>5.4. The category
\texorpdfstring{$\cat{Amp}_\star$}{Amp*}</h2>
A homomorphism $\phi\colon\mathcal{S}\to\mathcal{T}$ of inverse semigroupoids is <em>star-injective</em> if for all $a,b\in\mathcal{S}$, $\so(a)=\so(b)$ and $\phi(a)=\phi(b)$ implies $a=b$ (i.e., $\theta$ is injective on all fibers), and <em>star-surjective</em> if for all $t\in\mathcal{T}$ and all $a\in\mathcal{S}$, if $\so(t)=\so(\phi(a))$, then there exists $b\in\mathcal{S}$ with $\so(b)=\so(a)$ and $\phi(b)=t$.
...
<h3 class='subsection'>5.5. The functor \texorpdfstring{$\mathbb{K}\colon\cat{Amp}_\star^{\mathrm{op}}\to\cat{$\Sigma$-Ord}$}{K:Amp*op→Σ-Ord}</h2>
...
<!-- 5.170 -->
<div class='lemma' id='lem:functorkonmorphisms'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.170</span>
...
<!-- 5.171 -->
<div class='proof' id='proof171'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.171</span> <span class='envop'>(Proof of Lemma \ref{lem:functorkonmorphisms})</span>
...
The fact that $\mathbb{K}(\phi)$ preserves interpolators it equivalent to the equality <div class='equation'>
\begin{equation*}
\phi^{-1}(\so^{-1}(\so(B))\cap A)=\so^{-1}(\so(\phi^{-1}(B)))\cap\phi^{-1}(A)
\end{equation*}
</div>
for arbitrary $A,B\in\mathbf{KB}(\mathcal{T})$, and this is easily proven using star-surjectivity.\qedhere
</div>

<h3 class='subsection'>5.6. The functor \texorpdfstring{$\mathbb{P}\colon\cat{$\Sigma$-Ord}^{\mathrm{op}}\to\cat{Amp}$}{P:Σ-Ordop→Amp*}</h2>
...
<!-- 5.172 -->
<div class='lemma' id='lem:maponspectra'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.172</span>
    If $\mathfrak{F}\in\Omega(E(T))$, then $\theta^{-1}(\mathfrak{F})$ is nonempty, and the set <div class='equation'>
\begin{equation*}
\mathbb{P}(\theta)^{(0)}(\mathfrak{F})=\left\{e\in E(S):0\not\in e\theta|_{E(S)}^{-1}(\mathfrak{F})\right\}
\end{equation*}
</div>
...
<!-- 5.173 -->
<div class='proof' id='proof173'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.173</span>
...
<!-- 5.174 -->
<div class='lemma' id='lemma174'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.174</span>
...
<!-- 5.175 -->
<div class='proof' id='proof175'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.175</span>
...
<h3 class='subsection'>5.7. The natural equivalence</h2>
...
<!-- 5.176 -->
<div class='lemma' id='lem:subsetesofbasicarebasic'>
<span class='envidentifier'>Lemma</span> <span class='counter'>5.176</span>
...
<!-- 5.177 -->
<div class='proof' id='proof177'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.177</span>
...
\end{equation*}
    ...
    Therefore, as we can assume $U=X[e]$. Since $U\subseteq X[s^*s]$ then $U=X[e]\cap X[s^*s]=X[es^*s]$, i.e., we may assume $U=X[e]$ with $e\leq s^*s$. Let $z=s|(se)$, so <div class='equation'>
\begin{equation*}
X[z^*z]=X[(se)^*(se)]=X[es^*se]=X[e]=U.\qedhere
\end{equation*}
</div>
</div>

<!-- 5.178 -->
<div class='proposition' id='proposition178'>
<span class='envidentifier'>Proposition</span> <span class='counter'>5.178</span>
...
<!-- 5.179 -->
<div class='proof' id='proof179'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.179</span>
...
<!-- 5.180 -->
<div class='proposition' id='proposition180'>
<span class='envidentifier'>Proposition</span> <span class='counter'>5.180</span>
...
<!-- 5.181 -->
<div class='proof' id='proof181'>
<span class='envidentifier'>Proof</span> <span class='counter'>5.181</span>
...
<!-- 5.182 -->
<div class='theorem' id='theorem182'>
<span class='envidentifier'>Theorem</span> <span class='counter'>5.182</span> <span class='envop'>(Stone duality for ample inverse semigroupoids)</span>
