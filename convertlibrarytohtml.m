...
for i=1:length(entries)-1
  ...
  %find the authors
    ...
    if length(editor)==1;
      str_editor=['<br><b>Author</b>: ' editor{1}];
    else
      str_editor=['<br><b>Authors</b>: '];
      for k=1:length(editor)
  ...
  elseif strcmp(ref_type,'book')
    str_reference=[str_title str_author str_editor str_year str_series str_volume str_doi str_arxivID];
  elseif length(ref_type)>length('arxiv')-1 && strcmp(ref_type(1:length('arxiv')),'arxiv')
